...
- **`content_start` / `content_end`** are often auto-detected by `detect-bounds` via Brett's consistent phrasings (`grace and peace` at the start, `we practice...open table` / `come to the table` / `communion` at the end). Currently catches both bounds on ~48% of sermons, start-only on another ~19%. Auto-detected values get an inline `# auto: ... — verify` comment so reviewers know to check. Guest speakers and special services fall through to manual review.
...
4. **`npm run detect-bounds`** — heuristic auto-fill of `content_start` / `content_end` using Brett's consistent phrasings ("grace and peace" opening, "we practice...open table" / "come to the table" / "communion" closing). Guarded by a 15-minute minimum for start and a 10-minute minimum sermon length. Leaves fields null when phrasings don't match (guest speakers, special services). Inserts a `# auto: ... — verify` comment so reviewers know the value came from detection.
